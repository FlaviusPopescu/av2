...
function main(skip_detection)    
    ...
    gt = ev.gt1';
		...
		% gt is   3, 2 red    4, 5   blue    6 7 yellow
		% tracks is  1 2 yellow 3,4 blue     5,6 red

		yellow_d = sqrt( (gt(:,7) - tracks(:,1) ).^2 + (gt(:,6) - tracks(:,2)).^2    );
		blue_d = sqrt(  (gt(:,5) - tracks(:,3) ).^2 + (gt(:,4) - tracks(:,4)).^2     );
		red_d = sqrt(  (gt(:,3) - tracks(:,5) ).^2 + (gt(:,2) - tracks(:,6) ) .^2) ;
